...
- **FORK RESOLUTION (control loop, 2026-08-15 late — Corey asked
  ...
  **Bug-#2 scoped-decline slice DONE (`60540a6`, walker v85 — see the
  ✅ "Bug #2 LANDED" bullet above). Remaining on Corey's "go": push +
  cut 0.4.1 → A6 close → B7.**
- **Remaining (post-B6+riders, loop stopped — see the 🛑 2nd-stop
  ...
  0.5.0 (or interim 0.4.1) release · chapulin absorption +
  t_symex_decode WIP commit strategy · cpp-backend infra repair ·
  review items: A1 ref-alias/IRType, A5 minimal-pin, B2
  tryEvalInterval no-interval for iekConvIntWidth, B3 ScanError
  raisedTypeId through inlined callees, B3-discovered call-boundary
  UNSAT divergence class.
