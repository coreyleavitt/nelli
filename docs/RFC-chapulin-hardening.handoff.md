...
| R2 | CRIT | Q1 `tryRecognizeScanIdiom` accepts non-loop-invariant bound (dsl_parser.nim:2491-95, only itInt type-check); closed form evals bound once at entry → `while i<(n-i)` mis-rewritten → witness divergence | open | CONFIRMED (a6aa79cd) |
| R3 | HIGH | `Path.uncertain` taint convention-based: forkPath bool 4th arg, ~45 sites; `forkPath(…,false)` silently drops taint → latent false sxSat (root-cause sibling of R1) | open | design (no live trigger); split forkPath/forkPathTainted |
...
| R6 | MED | Q1 recognizer identifies "same var" by `.strVal` not symbol identity (dsl_parser 2508/2543/2550) → template/gensym mis-match | open | tighten to sameSymbol |
| R7 | LOW | Q1 AST-unwrap idiom duplicated 4× inline → unwrapHidden helper | open | design |
| R8 | LOW | P2a omitted non-scalar field → mkIntLit(0) mismatch → caught ValueError → sxUnknown (NOT a crash) but pollutes weInternalWalkerFault telemetry; classify at construction | open | REFUTED as crash (aa66ec68); downgraded from CRIT |
| R9 | LOW | coverage.nim exports macro-plumbing procs (registerEdgeSource/edgeSources) beside the one real entry point uncoveredSources | open | design |
| R10 | LOW | svIntToBV doc comment "always non-negative" false for parseInt("-5"); mechanism sound (int2bv mod 2^W) — doc fix | open | symex-core reviewer |
| R11 | LOW | C1 edgeSourceTable unguarded global + redundant per-eval re-registration | open | REFUTED as race (a17d66fc); latent-only, cosmetic |
| R12 | LOW | multiplexedDatabase drops secondary backend metadata on choice-seq collision (db.nim:658-664) | open | fuzz reviewer |
