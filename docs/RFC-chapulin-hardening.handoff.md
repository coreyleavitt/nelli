...
**Cluster 2 COMPLETE. M1–M4 landed.** Next unimplemented slice: **M5** (`min`/`max` —
add an `nnkIfExpr` case to `parseExpr` via synthetic let+read; min/max desugar to
if-expressions; also fixes if-expression sub-exprs generally; depends on SND-1✓;
CR-2a `Soft(t)` backstops transiently; size M). **⚠ M5 will invalidate CR-2a's RED
repro** (`(if c:1 else:2)+1` currently degrades to sxUnknown; M5 models nnkIfExpr → real
