...
- **Remaining:** A6 (BLOCKED on the Bug #2 fork above; checksum +
  docs clauses already done) · Track B B1–B7 (loop grinding these
  now — unblocked by the release per the hard ordering, orthogonal
  to variant classification) · chapulin absorption commit (Corey's
  call how to fold it into his WIP) · cpp-backend infra repair ·
  A1 ref-alias/IRType and A5 minimal-pin review items.
