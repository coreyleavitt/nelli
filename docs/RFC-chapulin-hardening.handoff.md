...
**RESUME after batch subagent returns:** review diff → NO version bump expected → control-loop run ONE full symex sweep (`scratchpad/psweep.sh <log> 220`, expect ~450 pairs, watch [[sweep-waiter-self-match]]: run_in_background, NO `&`) + ONE fcsweep (`scratchpad/fcsweep.sh <log> 180`) → if clean commit the batch as `refactor/docs(...): batch low-severity code-review cleanups` → update ledger → **DONE: report final review outcome to Corey.**
