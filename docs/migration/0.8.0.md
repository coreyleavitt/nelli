# Migrating to nelli 0.8.0

This file supersedes the retired per-RFC "downstream audit" format. It is
version-keyed, not RFC-keyed, and it addresses you as the maintainer of a
project upgrading to nelli, not a specific consumer of nelli. For the design
rationale behind this release — why the fields flipped from zero-fill to
declared defaults — see `docs/rfc/0010-config-discipline.md`. This file is
about what to do, not why.

There is no compile break in this release. Every literal that compiled before
compiles after. What changes is what those literals *mean*, and the change is
silent by construction — which is exactly the property that made the original
defect survive for years.

## The one-sentence version

If you construct any nelli configuration object with a **partial object
literal**, the fields you did not list used to arrive as zero and now arrive
as that type's documented default. If the zeros were what you wanted, write
them explicitly, and do so **before** upgrading.
...
# The reader trap in §3. A bare `var` still zero-fills; a literal no longer does.
...
from one that wrote the default. There is no flag nelli could add that would
tell you which of your literals changed. This table is the substitute — for
...
`false` **is** the zero value. A literal that wrote nothing got `false` and
now gets `true`.
...
**Three exceptions.** The type's umbrella comment claimed "0 = unlimited for
every field" and this section used to repeat it. That was false for four
fields whose enforcement sites were missing the `cap > 0 and` guard: an
explicit `0` exhausted the budget on the *first* use and degraded the run to
`sxUnknown`. **Two** are now fixed and genuinely honour 0
...
unreachable. Any symex query you wrote as a partial literal has been running
release-like. If it has assertions of the form "this query returns `sxUnsat`",
some of them may now be `sxRaised` — and that is the correct answer, arrived
at late.
...
type's defaults recursively. `FuzzSettings()` is no longer all-zero in that
one field. Behaviour is unchanged end-to-end: the value it now carries is the
one `resolved()` produced at the point of use.
...
1. **An explicitly all-zero `IntegerBiasConfig` is now honoured.** It used to
   ...
   draw. If you wrote the zeros deliberately, relying on the sentinel, you now
   get a different distribution. `resolved()` survives as a deprecated
   identity function, so calling it no longer rescues anything.
...
2. **Persisted symex witnesses go dark, once.** Symex verdicts are content-
   ...
   under that key. Settings whose meaning changed produce a *new* key, so
   stored witnesses become unreachable and are re-solved. They are never
   reused wrongly — this is an unannounced cache flush, not a correctness
   hazard. Budget a slower first run.
...
identifier, so the rename was unavoidable; the proc survives as a deprecated
shim so nothing fails to compile.

## 6. Toolchain

Unchanged. Nim ≥ 2.2.10 is still required, and specifically the declared
object field defaults this release is built on are a Nim 2 feature — a
toolchain pinned below that could not compile the library anyway.
