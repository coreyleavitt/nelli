...
- **Status:** Complete — closed at the 0-Critical/High/Medium review floor.
  ...
  push; that push is chapulin-repo work and is NOT tracked here — see the
  handoff's §Closed out). **All six owner decisions taken 2026-08-31** (see the handoff's
...
| **Open forks** | none. All six owner decisions taken 2026-08-31 with reasoning in the handoff; B7-2 fixed in `cac15e6`; N45 refuted 2026-09-03. The two residual items were closed out 2026-09-03 as other-repo work (chapulin push; softlink#17) |
...
| A6 | **LANDED (chapulin `7d637c5`, against the 0.4.1 release — DoD re-pointed 0.4.0→0.4.1 per the recorded fork resolution after Bug #2 made the 0.4.0 target unsatisfiable; riders below).** `t_symex_decode` un-voided 16/16: opData/opAck construct AND prove with the differential oracle (constructed packets witness-cross-checked against real `decode()`, exact field match), opOack constructs and proves construction-side, opError + opRrq/opWrq pinned as classified `sxUnknown` honest degrades (Track B territory, oracle coverage lands with B7). Full chapulin suite 34/34 + all seven `t_symex*` files green; milpa pinned `nelli@v0.4.1` (lock at `cdb4756`). Two NEW engine findings recorded (see the A6-rider-2 row and the handoff review items): constructor omitting a `seq[byte]` field degrades the whole proof to `sxUnknown` (worked around with explicit `@[]`), and `seq[byte]` element reads through a helper-proc call lose witness fidelity (all-zero on a genuine `sxSat` — silent-wrong-answer class, MUST fix before B7's oracle). Original scope: un-void `t_symex_decode` construction + EXTEND the differential oracle to compare constructed packets against real `decode()`. **Honest arm accounting (round-2 breadth):** at THIS gate only `opData`/`opAck` — the two arms with no `readCString`/`readOptions` on their path — produce real oracle-compared witnesses. The other three arms (`opRrq/opWrq`, `opError`, `opOack`) CONSTRUCT (A1/A3 land the constructor; no more macro error / void twin) but their surrounding walks stay `sxUnknown` pending Track B (B4 for the cstring scans, B5 for the chained mode scan, B6 for options) — their oracle coverage lands with B7. "All five arms" as a *witnessed* claim is a Track-B deliverable; Track A's claim is "all five arms construct, two prove end-to-end" | — | Chapulin suite green incl. oracle checks on opData/opAck, against the 0.4.0 release; the three deferred arms pinned as classified `sxUnknown` (not crashes) |

| A6-rider-1 | **LANDED (`ca51c3e` Bug #1 crash fix, no walker bump; `60540a6` Bug #2 walker v85; released as 0.4.1 `cdb4756`).** Bug #1: `classifyObjectRecordFields`' variant path crashed on EXPORTED fields (`nnkPostfix("*", name)` bare-`.strVal`) — shared `fieldNameStr`/`unwrapFieldNameNode` applied at the 3 variant-path sites. Bug #2: eager whole-type classification poisoned every proc returning a type carrying an unsupported field (`seq[(string,string)]` on untouched arms) to `sxUnknown` — replaced by per-field SCOPED DECLINE with read-taint: unsupported field → kind-marked placeholder (`isUnsupportedFieldPlaceholder`, R8 extension), fresh/length-zero allocation (also gives `itSeq[itTuple]` a walkable backing), SND-1 taint deposited only at actual READ sites (classified `seNestedSeqUnsupported`), `retBindEq` svSeq arm skips eq on flagged values (no-constraint over-approximation; taint owns honesty). Serialization companions: `emitIRType` itSeq arm (`tUnsupportedFieldSeq`) + `emitTyAndReader` placeholder case. Nested-seq CONTENT modeling stays the recorded non-goal | SW | `tests/tsymex_r6_bug2_scopeddecline.nim` 13/13 (poison-gone minimal repro flips back to real verdicts + SAT twin, honest-degrade read pins assert classification, retBindEq skip, A3 fork-per-tag untouched-tag pins, direct local touch degrades without crash, version floor); CR-2 `==` pin 84→85; ~35-file sweep green; `tsymex_CR2c_witnessreader_catchall`'s CR-2c-N3 migrated to the correct `sxSat` (untouched variant-arm `seq[Widget]` no longer whole-run-poisons) |

| A6-rider-2 | **LANDED (walker v86, render v10, 2026-08-16), required precursor to B7.** Chapulin's BLOCKER #12 finding ("`seq[byte]` witness extraction loses fidelity through a helper-proc read — an otherwise-genuine `sxSat` reports an all-zero witness") turned out, on isolated bisection, to be the visible SYMPTOM of a deeper SOUNDNESS gap, not a pure extraction/rendering issue — **a deviation from the row's own original framing, flagged rather than silently absorbed.** Root cause: `runtime.nim`'s `isCall` arm allocates one fresh, unconstrained `retSym` per call. An explicit `return expr` correctly ties `retSym` to the returned value via `retBindEq` (`isReturn`'s handler); but a callee that exits via IMPLICIT fallthrough after a CONDITIONAL, multi-statement `result = expr` assignment (`parseCalleeImpl`'s own documented "general parser path") never bound `retSym` to anything — it reached the caller totally free, letting Z3 satisfy any downstream comparison independent of the callee's actual computation. Confirmed as a genuine false-positive generator (not merely cosmetic) via a deliberately-unreachable target that proved a false `sxSat` pre-fix. Fix: mirror the closure-call path's own established idiom (`applyClosureGround`'s `retBindEq(funcApp, cp.env["result"])`) into the ordinary call-inlining `isCall` arm — every implicit-fallthrough exit with a bound `result` now asserts `retSym == result` (via the same `reconcileInt` bridge `isReturn` uses); a composite (non-scalar-wired) `result` kind degrades in-band to a classified `sxUnknown`, mirroring `isReturn`'s own existing degrade net, never a crash. **Version discipline note:** because the fix changes VERDICTS (a previously-false `sxSat` now correctly reports `sxUnsat`), `symexWalkerVersion` bumps 85→86 — NOT an extraction-only rider like B4-rider's precedent, despite the row's original framing assuming that shape. `renderAsChoicesVersion` bumps in lockstep, 9→10 | SW | New `tests/tsymex_r6_a6r_callwitness.nim` 12/12 (helper-mediated read witness content pin + real-function cross-check, inline control, the SOUNDNESS pin — a deliberately-impossible target is `sxUnsat` not a false `sxSat` — two-call-hop composition + cross-check, mixed inlined+helper composition + cross-check, constructor-omitting-seq[byte]-field regression guard unchanged, version floors); new TOT-1 composite-fallthrough-degrade row (`tests/tsymex_tot1_totality_corpus.nim`); CR-2 `==` pin 85→86 / render 9→10; ~35-file sweep green (round-6 A0–A4/A6/B0–B6/Bug#2, CR-2a/b/c catchalls, multivariant IR/typebridge/walker/witness, `retest_c6_tuple_chain`, `r5_tuple_return`, `funcdef_callee`, `phase15_N3_scan_boundary`, `phase15_g8/g10`) |

**Exit gate A: ✅ CLOSED 2026-08-16** — A6 observed green against 0.4.1 (superseding the 0.4.0 target per the fork resolution). Original text: A6 observed green. Release 0.4.0. Checked non-dependencies
...
| B4 | **LANDED (walker v82).** `tryRecognizeAccumulatingScan`/`tryMatchAccumulatingScanIdiomShape` (`dsl_parser.nim`) — a THIRD sibling of Q1/B0's `tryRecognizeScanIdiom` and B3's `tryRecognizeScanPairIdiom`: B3's early-return-on-match shape with a THIRD body statement that ACCUMULATES the pre-terminator bytes into a string as the loop advances (`while i < s.len: (if s[i] == lit: return <expr>); acc.add(char(s[i])); inc i` — the `readCString` family). Lowers to the SAME `iekStrFind` 3-arg closed form, reusing B0's not-found/OOB split verbatim, plus one new binding for the accumulated payload: `<acc> = <acc's entry value> & iekStrSubstr(<s>, <i>, p − 1)` — the pinned inclusive-hi formula, generalized with a leading concat of `<acc>`'s own entry value so the closed form stays sound without inspecting the loop's preceding statements. Body statement count (1 vs 2 vs 3) keeps the three recognizers mutually exclusive by construction; `tests/tsymex_r6_b3_scanpair.nim`'s "B3-5" trip-wire (the exact 3-statement shape, previously pinned `sxUnknown`) MIGRATED to the real verdict B4 now proves — a sound capability upgrade. Two supporting fixes landed alongside the recognizer, both required for the closed form to actually WALK rather than just parse: (1) `collectIntOffsetParams` (ADR-0027's recorded lift, left unbuilt by B1 since Q1/B0/B3 never needed it) — `iekStrSubstr`'s LOW bound must be `svInt` (its own runtime arm declines a BV-represented bound, the CR-17 non-termination class), so a top-level int param whose def-use reaches an accumulating-scan's index now allocates `svInt` instead of the `itInt` default, traced through at most one local rebind AND one direct call boundary (`readCString`-shaped helpers are naturally called through a wrapper, e.g. chapulin's own `readOptions`); (2) this per-param promotion surfaced a live CRASH (`FieldDefect` on a BV-only field access) in `overflowCond`/`lowerArith`, which had never before seen a MIXED `svInt`/`svBV*` operand pair — `lowerArith` now calls `reconcileInt` at its top, mirroring `lowerCmp`'s own existing call. Also fixes B1's flagged witness-reader gap: `readSeqUInt8` (`runtime.nim`) now routes a string-backed `seq[byte]` param's model value through `RawWitness.strVals` before falling back to the array representation. `renderAsChoicesVersion` bumped 7→8 in lockstep (the reader fix is a genuinely changed witness CONTENT reaching `renderAsChoices`, the M1 "5" bullet's own precedent); two other files' own stale exact `renderAsChoicesVersion == "7"` pins (`tsymex_r1b_shortcircuit_oob.nim`, `tsymex_r14_continue_guard.nim`) converted to `>=` floors, matching the existing `symexWalkerVersion` floor convention. **Discovered, flagged, NOT fixed here (out of scope — pre-existing, orthogonal to B4):** witness EXTRACTION for any solved string whose model requires an embedded NUL byte (`'\0'`, chapulin's own real delimiter) comes back corrupted — the byte is replaced by the 5-character text of its own SMT-LIB escape spelling (`\u{0}`), reproduced with minimal repros using only pre-existing `iekStrAt`/`iekStrFind` (no B4 code). `'\0'` proves reachable via STATUS alone (pinned); the test suite otherwise uses `':'`, matching B3's own delimiter precedent, for every pin needing witness CONTENT | SW | New `tests/tsymex_r6_b4_readcstring.nim` 10/10 (symbolic-start SAT + witness cross-check, dedicated inclusive-hi empty-payload SAT + cross-check, not-found-clamp UNSAT companion, not-found `ScanError` fork, negative-start OOB `IndexDefect`, non-`.len`-bound trip-wire stays `sxUnknown`, the real `'\0'` delimiter proves status-only, string-backed `seq[byte]` witness-reader fix, version floor); CR-2 `==` pin 81→82; mandated regression sweep (new file + A0–A4/B0–B3 + retest_c6_tuple_chain + q1_scanlift + r1b + CR2 + TOT-1 + N3_scan_boundary + every scan/find/lift/witness-named test, 24 files) green except the pre-existing `S3_strindex` red (confirmed unchanged, "s.high == 4"). Interim INT-1 deferred (needs-Corey, per the standing bundle process) |
| B4-rider | **LANDED (no walker version bump — extraction-only fix), required precursor to B7.** Fixes B4's flagged pre-existing bug: witness EXTRACTION for any solved string whose model contains an embedded NUL byte corrupted the byte to the 5-character text of its own SMT-LIB escape spelling (`\u{0}`). Root-caused with a `_deps`-only repro, no nelli code involved (a bare `mkString("\x00")` round-tripped through nim-z3's `evalStr` alone reproduces it): nim-z3's `evalStr`/`toStr` wraps `Z3_get_lstring`, which mis-renders a string containing an embedded NUL specifically — empirically NOT a general "any escape-needing byte" defect (standalone backslash/quote/control-byte/0x7f/0xff witnesses all extract correctly through the unfixed path; they only came back wrong in B4's own `'\0'`-delimited pin because they shared a witness with a real NUL, whose 5-char expansion shifted every later byte). Fix: `extractLeaf`'s `svString` arm (`runtime.nim`) now calls a new `evalStrBytes` helper built on `getStringLength`/`getStringContents` (`Z3_get_string_length`/`Z3_get_string_contents`) — a SEPARATE already-bound Z3 API (`z3/strings`, already re-exported through nelli's `import z3`) returning raw codepoints rather than a rendered string; confirmed correct for every tested byte on both a bare literal AST and a solver-model evaluation. nelli's own byte-faithful string invariant (Phase 15 S3/ADR-0006: every free `string` constrained to `(re.range '\x00' '\xff')*`) guarantees every codepoint is in `[0, 255]`, so the codepoint→Nim-byte mapping is exact and total. Verdicts are unchanged (`symexWalkerVersion` stays 84); `renderAsChoicesVersion` bumps 8→9 (genuinely new witness CONTENT for affected strings, per B4's own "8" precedent) — `tsymex_phase15_CR2_cachekey.nim`'s exact pin updated. B4's own real-`'\0'` pin (`B4-8`, `tests/tsymex_r6_b4_readcstring.nim`) strengthened from status-only to content assertions + a real-function replay cross-check | — | New `tests/tsymex_r6_nulwitness.nim` 15/15 (embedded-NUL-at-known-position content pin, NUL-delimited B4-shape scan content pin + cross-check, standalone low-byte/backslash/quote/0x7f/0xff pins confirming the bug's NUL-specific scope, mixed-byte-class witness, plain-ASCII regression, version pins); B4-8 strengthened (2 new content/cross-check pins); mandated regression sweep (new file + b0/b1..b6 + q1_scanlift + CR2 + r1b + r14_continue_guard + every witness-named test, 15 files) green except the pre-existing `S3_strindex` red (confirmed unchanged, "s.high == 4") |
| B5 | **LANDED (walker v83).** Chained composition (catalog #6): a SECOND lifted scan (B3/B4) whose start offset is a FIRST scan's own RETURNED result. Zero-iteration chain seeds (a not-found first scan feeding a genuinely-empty second scan) composed for free, inheriting B0's guarded-entry discipline by construction — no fix needed there. But the faithful catalog-#6 shape (`let (_, p1) = readCStringHelper(s, 0); discard readCStringHelper(s, p1)`) hit a REAL gap: `retBindEq`'s fresh call-return placeholder (`freshRetSym` → `allocateSym`) allocates every `itInt` tuple field at its type-driven BV default regardless of what the callee actually computed — `reconcileInt` (CR-9(c)) only widens the pair used in the `retBindEq` equality constraint itself, never the caller's own env binding for the destructured local — so a chained scan's second offset stayed BV-represented and failed `iekStrSubstr`'s CR-17 Int-sortedness check. Fixed via new `calleeIntOffsetReturnPositions`/`scanOffsetReturnPositions`/`offsetShapedElem` (`dsl_parser.nim`): trace which tuple positions of a callee's own recognized B3/B4 closed form are genuinely Sequence-theory Int, threaded onto the `isCall` IR statement (`IRStmt.retIntOffsetPositions`, new field) so `allocateSym`'s `itTuple`/`itInt` arms allocate `svInt` directly there — mirroring `IRParam.isIntOffset`'s existing top-level-param promotion, applied at the call-RETURN end of the data flow. Two companion fixes landed in the same cycle: `parseCalleeImpl` now also marks a CALLEE's own `isIntOffset` params (previously only `parseProc*`'s top-level entry loop did), so a LITERAL scan-offset argument gets an svInt proto instead of `intLitProto`'s BV default at the call-argument-lowering site (`runtime.nim`); and `emitStmt`'s `isCall` NimNode-literal reconstruction now serializes the new field (an unserialized IRStmt field silently reverts to `mkCall`'s `@[]` default at compile time — the exact failure mode the TDD RED run first surfaced). Purely additive: an untraced position keeps the pre-existing BV default, so no already-decided verdict changes anywhere in the corpus | SW | New `tests/tsymex_r6_b5_chained.nim` 9/9 (faithful catalog-#6 repro SAT+UNSAT companion retiring the finding; two-sequential-readCString `filename`/`mode` composition with witness cross-check; zero/degenerate chain-seed SAT+UNSAT companion, no clamp resurrection; non-`.len`-second-bound trip-wire stays `sxUnknown`; B4-then-B3 mixed sibling composition; version floor); mandated regression sweep (b0/b3/b4/q1_scanlift/retest_c6_tuple_chain/r1b/CR2/TOT-1/N3_scan_boundary) green except the pre-existing `S3_strindex` red; supplementary 14-file sweep (variant/retbind/construct-interaction/generic-dispatch/closure/tuple-return tests, to cover the call-argument/callee-param machinery this fix touches) green; CR-2 `==` pin 82→83 |
| B6 | **LANDED (walker v84).** Option-region membership (the `readOptions` pair-loop): region ∈ `((nonzero)* "\0")*` — **STAR inner segments, not plus (round-2 depth):** the real `readCString` returns empty strings freely and `readOptions` accepts mid-region empty keys and all empty values, and the canonical double-NUL terminator is itself an empty segment; `(nonzero)+` would fail to certify exactly the inputs a property search generates. New `tryRecognizePairLoopIdiom`/`tryMatchPairLoopIdiomShape` (`dsl_parser.nim`) — a FOURTH sibling of Q1/B0/B3/B4's scan-idiom family — recognizes the 5-statement `readOptions` pair-loop body (two chained B4-shaped calls, empty-key break, fold, index advance) and replaces the WHOLE loop with a two-way fork on a new `iekStrInOptionRegion` boolean (`types.nim`/`runtime_strings.nim`), built directly from nim-z3's `range`/`star`/`concat`/`matches` sequence-regex primitives — no unbound Z3 primitive; no nim-z3 change needed. Membership is the LOOP-SAFETY invariant, not a per-pair functional-correctness claim: a non-member region (truncated tail) falls back to the SAME `mkShortCircuitWhile` k-unroll every unrecognized loop shape already takes (reaching the pre-existing modeled ScanError raise arm — no new decline machinery), never all-or-nothing. **Depth finding (empirical, this engine build):** BOTH branches must omit the fold (`<pairs>.add(...)`) — not only the member branch — because `itSeq[itTuple[string,string]]` has no backing in `allocateSeqDataRaw` this cycle (the same gap the A6 exit-gate recorded for `seq[(string,string)]` as a formal param, here hit as a local), and this engine's `isIf`/`isWhile` walker descends into every branch UNCONDITIONALLY with no feasibility pre-check — an unmodeled construct on a syntactically-present-but-dynamically-infeasible branch is caught only by `runSymexImpl`'s single top-level handler, which returns `sxUnknown` unconditionally and discards any already-recorded `sxSat` witness. Dropping the fold from both branches is literally the ADR-0028 text ("the no-defect proof for the whole option arm WITHOUT MODELING THE FOLD") and is sound for every pin (no verdict depends on `pairs`' content). A one-line companion rider in `lowerSeqLit` (`runtime.nim`) makes the EMPTY-literal seq declaration (`var pairs: seq[(string,string)] = @[]`) walkable for any element type by skipping `allocateSeqDataRaw` when the literal has zero elements (a length-0 seq's `isIndex` OOB bound is unsatisfiable for every index, so the backing array is never read on any live path) — required for the accumulator's own declaration to be walkable at all, independent of the fold. `i`'s Int-sortedness (`iekStrInOptionRegion` reuses `iekStrSubstr`'s CR-17 discipline — a BV-represented bound declines rather than bv2int-bridging arithmetic into a Sequence-theory query) rides the EXISTING B4/B5 `collectIntOffsetParams` "wrapper" promotion unchanged — no new promotion machinery. dt-bounded calibration: no divergent query encountered — every pin decided in seconds | SW | `tests/tsymex_r6_b6_optionregion.nim` 8/8 (main defect proof SAT + RED trip-wire baseline retiring it; empty-key/empty-value/double-NUL-terminator star-segment pins, each past the 5-iteration k-unroll horizon; truncated-region fallback ScanError-raised pin; non-`.len`-bound trip-wire stays `sxUnknown`; version floor); CR-2 `==` pin 83→84; mandated regression sweep (new file + b0/b1/b2/b3/b4/b5 + q1_scanlift + retest_c6_tuple_chain + r1b_shortcircuit_oob + CR2 + TOT-1 + N3_scan_boundary + r6_a0..a4, 19 files) green except the pre-existing `S3_strindex` red (confirmed unchanged, "s.high == 4") |
| B7-rider | **LANDED (walker v87, render v11), required precursor to B7's resume.** B7's first attempt (2026-08-16, against local HEAD `d54f85a`, walker v86) blocked on TWO gaps, both closed here. **LEG 1 (BLOCKER A):** B3/B4/B5/B6's closed-form recognizers (`tryRecognizeScanIdiom`/`tryRecognizeScanPairIdiom`/`tryRecognizeAccumulatingScan`/`tryRecognizePairLoopIdiom`, `dsl_parser.nim`) hard-gated the receiver on `classifyType(...).ty.kind == itString` and never consulted B1's `ctx.stringBackedParams` — a `seq[byte]` receiver never got a closed form, confirmed the RFC decomposition hole B3's own row flagged ("deliberately NOT widened … per B1's own scope note") and no later slice picked up. Widened via a new shared `scanReceiverOk`/`scanDelimiterChar` (`dsl_parser.nim`): a receiver qualifies if it is itString OR string-backed per `ctx.stringBackedParams`; the delimiter gate widened in lockstep to accept a byte-range literal (`0'u8`/`byte(0)`/`0x00`) alongside a char literal. A SECOND, deeper gap surfaced while landing this: B1's OWN classifier (`collectStringBackedByteSeqParams`) only ever detected a Q1-shaped (and-guard) consuming loop when deciding whether a `seq[byte]` param qualifies — but chapulin's ACTUAL `readCString`/`readOptions` shapes are B4/B6-shaped, never Q1-shaped, so those params would never have been classified string-backed at all, no matter how far the four recognizer gates were individually widened; the classifier's candidate walk now tries all four shape predicates per loop (B6's own shape-match, `tryMatchPairLoopIdiomShape`, had its receiver type gate MOVED out into the recognizer — it previously embedded the itString check inside the shape-match itself, a chicken-and-egg blocker for the classifier, which must call the SAME predicate before `ctx.stringBackedParams` exists). A THIRD gap: `runtime.nim`'s `isCall` arm lowers a call's actual argument once, in the CALLER's own env, then binds it DIRECTLY into the callee's env with no representation bridge — unlike `IRParam.isIntOffset` (int/BV are fungible via a proto), itString/itSeq are different Z3 sorts with no lossless reinterpretation, so a caller whose own top-level `seq[byte]` param has no qualifying loop of its own (the scan lives entirely inside a callee it invokes — the common case) would allocate array-modeled while the callee's inlined body expects `svString`. Closed via a NEW one-level call trace in `collectStringBackedByteSeqParamsImpl`, mirroring `collectIntOffsetParamsImpl`'s existing "wrapper" promotion verbatim (same `visiting` cycle guard, same "at most one direct rebind" `findRootParam` trace). Two smaller latent bugs surfaced and fixed along the way, both pre-existing and never previously triggered (no corpus entry needed them): `tryMatchAccumulatingScanIdiomShape`'s accumulator-arg unwrap only recognized `char(<s>[<i>])` as `nnkCall`/`nnkCommand` — Nim's typed AST actually represents an explicit `char(...)`/`byte(...)` conversion as `nnkConv` (confirmed via `treeRepr`), so this was dead code until a `seq[byte]` receiver's `.add(char(data[i]))` accumulator statement (needed since `data[i]: byte`, not already char-typed) exercised it for the first time; both `scanDelimiterChar`'s own `byte(<lit>)` unwrap and the accumulator-arg unwrap now handle `nnkConv`. **LEG 2 (companion char-widening witness-corruption bug):** root-caused, and the chapulin handoff's own hypothesis ("witness EXTRACTION corrupts") found INCORRECT — `evalStrBytes`/`getStringContents` (B4-rider's own extraction chokepoint) faithfully report whatever Z3's model contains. The real defect is a PARSE-TIME MODELING GAP one call site removed from extraction: `char` was never a member of `intTyNames`/`normalizeIntTyName`'s alias map (unlike `byte`), so `isIntFamilyName("char")` was FALSE and a widening conversion off a char (`uint16(s[i])`, chapulin's own opcode-dispatch header-read shape) silently fell through `dsl_parser.nim`'s `nnkConv` arm to a bare identity pass-through — DROPPING the widening entirely, so `hi`/`lo`/`combined` all stayed 8-bit despite their DECLARED 16-bit Nim type (`isLet`'s walker arm binds whatever `lower()` returns for a non-literal RHS with no width coercion). Comparing an 8-bit `combined` to a 16-bit literal truncated the literal to its low byte at `coerceIntLit`'s own literal-shaping step, so the checked property silently degenerated to "the low byte alone matches", leaving the high byte totally unconstrained — `sxSat` was technically correct but the reported witness reflected Z3's free choice for the unconstrained byte, not the value the property actually depended on (confirmed empirically: the reported witness did not reproduce the target when replayed through the real widen+shl+or expression). Fix: `normalizeIntTyName` now maps `char` to `"uint8"`, exactly like `byte` — semantically exact, `char` is ordinally an 8-bit UNSIGNED value under a distinct (non-alias) name. `symexWalkerVersion` bumps 86→87 (both legs are verdict-class fixes, not extraction/rendering-only — a `seq[byte]` receiver moves from an unrecognized k-unroll to a real verdict, and a char-widened property moves from a silently-truncated check to the correct one); `renderAsChoicesVersion` bumps 10→11 in lockstep (both legs change already-`sxSat` witness CONTENT for affected SUTs) | SW | New `tests/tsymex_r6_b7r_bytescan.nim` 26/26 (all four recognizers proving through a `seq[byte]` receiver with SAT+UNSAT companions; byte-literal delimiter spellings `0'u8`/`byte(0)`/`0x00` incl. the real terminator value; a B5-shape helper-call composition on a `seq[byte]` receiver; B6's pair-loop widened; mutation-fallback veto still applies; non-`.len`-bound trip-wire stays `sxUnknown`; sibling exclusivity confirmed on a byte receiver; the LEG 2 char-widening repro with content + real-function cross-check + a full-16-bit-width existence proof + a char-narrowing-decline regression pin; version floors); CR-2 `==` pin 86→87 / render 10→11; ~30-file mandated regression sweep (b0–b6, nulwitness, bug2_scopeddecline, a6r_callwitness, q1_scanlift, r1b_shortcircuit_oob, r14_continue_guard, CR2, TOT-1, retest_c6_tuple_chain, N3_scan_boundary, every witness-named test, r6_a0–a4/a6_exported_field_names) green except the 3 documented pre-existing reds (`S3_strindex`, `C4_hof` ×2, the 3 MSVC C2026 audit files — confirmed unchanged) |
| B7r2 | **LANDED (walker v88), path-scope rider — required precursor to B7's resume.** B7's second attempt (2026-08-16, against local HEAD `8b3c593`) isolated BLOCKER B7-1's three breakers and BLOCKER B7-2; this rider closes two of the three B7-1 breakers with root-cause fixes and finds the general "branch-scoped classified degrade" architecture the RFC's own design section proposed for all four UNSOUND on this engine's C backend — see the handoff's BLOCKER entry for the full escalation. **Breakers (a)/(b) (if-wrapped / construction-followed pair-loop), ONE root cause:** B6's closed form (`iekStrInOptionRegion`) requires its start/bound operands `svInt`-represented (the CR-17 discipline); `collectIntOffsetParams`'s existing promotion only traces a loop counter back to a FORMAL PARAM through a bare-symbol rebind chain — chapulin's real `decodeOackTwin`/`readOptions` shape seeds the counter from a LITERAL (`var pos = 2`, straight after a header-length check) with no param to trace to, so it stayed BV-allocated and failed the CR-17 check regardless of if-wrap or downstream construction (empirically isolated: a faithful minimal repro with the counter fed from a genuine `start: int` formal param, matching `tsymex_r6_b6_optionregion.nim`'s own pinned shape, does NOT reproduce either breaker — only the literal-seeded variant does). Fixed via a new companion parse-time collector, `collectIntOffsetLiteralLocals` (`dsl_parser.nim`): finds a recognized scan/pair-loop counter whose OWN `var`/`let` declaration initializes it with a bare int literal (not traced through any param — a literal's value is already known at parse time, so `svInt`-representing it carries none of a param's def-use tracing risk) and marks it into a new `ParseCtx.intOffsetLiteralLocals` field, populated at the same site/scope as `stringBackedParams`; a new `IRStmt.isLet.lIsIntOffsetLocal` field (threaded through `emitStmt`'s compile-time→runtime round trip, the B5 "unserialized field reverts to default" lesson) is consulted by the `isLet` walker arm to select an `svInt` proto for a literal RHS instead of `intLitProto`'s BV default — a no-op for a non-literal RHS (the proto is ignored by `lower`'s `iekVar` arm either way). **Breaker (c) (call boundary), a SECOND, INDEPENDENT root cause:** a helper proc RETURNING `seq[(string,string)]` (chapulin's real `readOptions`/`readOptionsTwin` signature) poisoned any caller merely BINDING the call's result, even when immediately discarded — Bug #2's per-field scoped-decline placeholder (`isUnsupportedFieldPlaceholder`) was scoped to declared object/variant RECORD FIELDS only (`classifyObjectRecordFields`); a bare local/param/call-RETURN of an unsupported-element seq type still hit `allocateSeqDataRaw`'s unconditional raise. Generalized `allocateSym`'s `itSeq` arm to take the SAME placeholder branch whenever `not isBackedSeqElemTy(ty.seqElemTy)`, regardless of field-vs-bare origin; a NEW walk-time guard at `isIndex`'s `svSeq` case deposits the SAME SND-1 taint on an ACTUAL indexed read of such a placeholder (the read-safety companion the generalization needs, since a bare value has no static field-access site for `dsl_parser.nim`'s existing `nnkDotExpr` interception to catch). Both fixes are CONFIRMED CAPABILITY UPGRADES, not merely crash-avoidance (isolated probes: a literal-seeded, if-wrapped pair-loop past the 5-iteration k-unroll horizon now proves via the closed form; a call-boundary `seq[(string,string)]`-returning helper's caller now reaches a target past the call; the full opOack-faithful composite — header check, opcode range check, `if wireOp == 6:` dispatch, call-boundary `readOptions`, variant construction — reaches its post-construction target end-to-end, and a genuine defect on a DISJOINT sibling arm of the SAME composite proc is still found). **BLOCKER B7-2 (case/else-raise sibling poisoning) is a THIRD, GENUINELY DIFFERENT mechanism** (CR-2a's `feUnsupportedExprKind` on case-as-expression, unrelated to int representation or seq backing) that the RFC's own "branch-scoped classified degrade" architecture (catch a construct-gap exception at the `isIf`/`isWhile`/`isCall` walk boundary, taint only that branch via SND-1, let siblings keep exploring) would address ARCHITECTURALLY. That approach was PROTOTYPED (a new common `SymexConstructGapError` base for the 19 construct-gap carriers + `try`/`except` around the four walk-boundary call sites) and found UNSOUND on this engine's C backend: wrapping ANY of those recursive `walk()` calls (which return `seq[Path]`, holding refcounted Z3-AST fields) in `try`/`except` — even a SINGLE, non-re-raising catch, at just ONE of the four sites — causes the underlying classified-degrade exception to never be raised AT ALL (not corrupted-but-visible: `w.sawUnknown`/`w.walkDegradeErrors` verifiably never get set, confirmed via an in-band print immediately before `runSymexImpl`'s own verdict computation, reproduced identically whether catching the new common base type or one of the 19 existing concrete types directly, independent of `--forceBuild`/nimcache state, and independent of whether the handler re-raises or fully consumes). This is a NEW, MORE SEVERE manifestation of the SAME pre-existing ADR-0020/CR-1c finding this codebase already recorded and architected around (the single top-level `runSymexImpl` catch) — not a new discovery from scratch, but confirmation the constraint is stricter than "don't catch at EVERY frame": even ONE non-top-level catch around a `seq[Path]`-returning recursive call is unsafe on this toolchain (Nim 2.2.10-patched, C backend, ORC, `--exceptions:goto`, `--threads:on`). The prototyped base-type/catch changes were fully reverted; BLOCKER B7-2 and the fully-general sibling-survival architecture remain UNFIXED this slice, pinned as an honest, UNCHANGED classified decline (a regression trip-wire, not a claimed fix) — asserted on BOTH status and classification so a future engine fix that closes this gap flips the pin red, signalling "tighten", never silent staleness. `renderAsChoicesVersion` is UNCHANGED (11) — no witness CONTENT changes; both landed fixes are proof/reachability capability upgrades, not extraction changes | SW | New `tests/tsymex_r6_b7r2_pathscope.nim` 11/11 (breaker (a) SAT past the k-unroll horizon + a non-`.len`-bound RED trip-wire retiring it; breaker (b) unconditional loop + construction reaches its target; breaker (c) call-boundary reachability + a clean-sibling-defect-still-found companion; the opOack-faithful composite end-to-end + its own clean-sibling companion; BLOCKER B7-2's shape pinned as an unchanged classified decline with status+classification asserted; a genuinely-unmodeled `Table[string,seq[int]]` regression trip-wire; version floors); CR-2 `==` pin 87→88; ~43-file mandated regression sweep (new file + b7r_bytescan, b0–b6, nulwitness, bug2_scopeddecline, a6r_callwitness, q1_scanlift, r1b_shortcircuit_oob, r14_continue_guard, CR2_cachekey, TOT-1, retest_c6_tuple_chain, retest_c5b_unknown_errors, N3_scan_boundary, a0–a4 + a6_exported_field_names, phase6_while/for/case/break, E-series (E2a/E3/E5/E6), every witness-named test) green except the 5 documented pre-existing reds (`S3_strindex`, `C4_hof` ×2, 3 MSVC C2026 audit files — confirmed unchanged). A TOT-1 corpus row for the new `isIndex` decline site was ATTEMPTED and WITHDRAWN: the isolated shape classifies correctly (confirmed standalone, `seNestedSeqUnsupported`), but embedding it as the ~20th sequential `symexFind` call in that file's own binary triggers a cumulative Z3 `Z3_MEMOUT_FAIL` unrelated to the row's own correctness (reproduced across three simplifications, including a zero-iteration variant) — a solver-resource characteristic of that specific file, not a correctness gap; the site is instead pinned via this slice's own dedicated test file |
| B7 | **LANDED (chapulin `a4fc7c3`, against the released nelli 0.5.0 `b1ea6af`, walker v88 — three passes: five-opcode oracle at v87, opOack unblocked by the B7r/B7r2 riders, final unification at v88).** ONE natural `decodeTwin` (all six wire opcodes / five arms, `seq[byte]`-native, matching real `decode`'s dispatch shape); all five arms prove `sxSat` with witnesses cross-checked against real `decode()` (opRrq/opWrq via genuine chained filename→mode scans; opOack via option-region membership through dispatch + construction, options content unmodeled per the committed gate); `decodeFullTwin` differential oracle green on hand-picked + property vectors; checksum re-probe CLOSED (const-fold confirmed resolved, 8/8). Honest-scope note: the whole-proc unconstrained no-defect search over the unified twin is `sxUnknown` (the membership proof's NON-member fallback k-unrolls at `maxLoopUnwind=5` — B6's committed scope never claimed index-safety for an unconstrained non-member region; verified pre-existing on the isolated opOack arm, not a merge artifact); every specific-target assertion is green. Stretch goal (real `decode` direct to `symexFind`): still `sxUnknown`, attributable solely to the deferred case/else-raise gap (see below). Recorded deferral: **B7-2** — a `case`-match over scanned content with `else: raise` poisons a disjoint sibling branch (CR-2a `feUnsupportedExprKind` on case-as-expression); the branch-boundary `try`/`except` fix is UNSOUND on this C backend (exception silently elided — ADR-0020/CR-1c class, empirically confirmed), so the fix is an in-band signaling refactor scoped as its OWN future round. Original scope: Chapulin migration — expanded DoD (round-2 breadth): twins to `seq[byte]` (drop `atByte`'s mask + the `seq[int]` typing across all four helper twins); UNIFY the three separately-scoped decode twins (`decodeFixedArmsTwin`/`decodeFixedArmsErrorTwin`/`decodeOptionArmTwin` — split precisely to dodge gaps B3–B6 close) into one natural twin; re-derive their 9 green assertions; extend the differential oracle to the unified twin (the deferred 3 arms from A6); re-probe `t_symex_checksum` (const-fold action recorded at v69, still outstanding). Passing the REAL `protocol.decode` to `symexFind` directly is a recorded STRETCH GOAL, not gated — the twin remains the vehicle | — | The natural unified decode twin proves in chapulin's suite against 0.5.0; oracle covers all five arms; checksum re-probe recorded in chapulin's findings doc |

**Exit gate B: ✅ CLOSED 2026-08-16** — the fully-natural single-param decode
proof observed green in chapulin's suite against released 0.5.0 (`b1ea6af`,
walker v88): widened-BV header math, closed-form scans, and the
option-region membership on one string-backed param, all through one
unified twin (chapulin `a4fc7c3`). Original text: the fully-natural
single-param decode proof — widened-BV header math, closed-form scans, and
the option-region membership on one string-backed param. Release 0.5.0.
