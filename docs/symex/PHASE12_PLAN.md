...
| 9 | UNSAT findings re-derived on every call (not cached) — O(queryTimeoutMs) per target on warm runs | 12 | Documented honestly in README (not "cheap"); could be cached if perf demands | A SUT with many UNSAT-prone targets shows latency on warm runs |
