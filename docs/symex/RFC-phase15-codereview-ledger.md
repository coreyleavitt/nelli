...
|----|-----|--------|-----------|---------|--------|
| CR-1 | High | open | runtime.nim:6144-6154 | Closure-body heap writes (`p[]=v`) never merged back to caller path → **false UNSAT** for refs mutated inside a closure | HOLDS (verifier: no return-merge after closure descent; comment "inert until R4" stale, R4 shipped) |
...
| CR-3 | Med | open | runtime.nim:2571-2580 | float→int out-of-range: silent unsound witness, no sxUnknown/hint (won't round-trip; Nim RangeDefects) | HOLDS (documented "unsoundness window"; no guard) |
| CR-4 | Med | open | runtime.nim:2575-2579 | `convWidth` set by parser but never read; `int32(f)` modeled as 64-bit truncation → wrong witness in (INT32_MAX,INT64_MAX] | HOLDS |
| CR-5 | Med | open | runtime.nim:6146 | closure-descent `descentBase.liveRefs` empty → closure-body `new T` not asserted distinct from caller refs → spurious aliasing witness | HOLDS |
| CR-6 | Med | open | runtime.nim:2363 | `cmpFloat` `doAssert a.kind==b.kind` crashes (not sxUnknown) for float32-var vs float64-var compare (Invariant-3) | PARTIAL (narrow case holds; retBindEq path refuted) |
