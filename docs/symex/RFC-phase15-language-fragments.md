...
1. **Cluster Z** — bump `nim-z3` pin to v2.0.0. The v2.0.0
   migration doc's "Nelli sync note" already confirmed that
   none of the eight symbol-level renames appear in nelli's
   source tree (nelli's only nim-z3 import is the umbrella
...
6. **Single-source-of-truth version constants.**
   ...
   `src/nelli/smt/canonicalize.nim`. Every consumer
...
> nelli is unaffected by the 8 breaking renames, and lay the
...
- `src/nelli/strategies/tuples.nim` — extend the macro to handle
  named-field `nnkIdentDefs` nodes.
- `src/nelli/strategies/floats.nim`, `strings.nim`, `lists.nim`,
  `tables.nim`, `sets.nim` — populate `constraintDigest` from
  constructor parameters.
- `src/nelli/smt/db.nim` — add skip-load guard: entries whose cache
...
`bvToFpBits`; `mkRegexAll` → `mkRegexAllChar`) appear in nelli source.
Per the nim-z3 MIGRATION-1.x-to-2.0.md "Nelli sync note", the Lens 4
grep at the 2.0.0 tag confirmed none of the renamed symbols exist in
nelli's tree — so the full migration delta is a single pin-bump with no
source edits.
...
as stale references in nelli's transitive build.
...
- No source files under `src/nelli/smt/` require edits. The only
  nim-z3 import in the smt layer is `import z3` in
  `src/nelli/smt/runtime.nim` (umbrella import — unaffected by the
  submodule renames because nelli never imported `z3/string`,
...
- [ ] `nim c` of `src/nelli/smt/runtime.nim` succeeds against the
...
Added to the public API surface (`src/nelli/smt/runtime.nim` or a new
`src/nelli/smt/settings.nim` — whichever the GREEN plan specifies):
...
- `src/nelli/smt/types.nim` — add `SymexErrorSeverity`, `SymexErrorKind`,
...
- `src/nelli/smt/dsl_typebridge.nim` — add `char` branch and
  `nnkSinkTy`/`nnkLentTy` strip to `classifyType`.
- `src/nelli/smt/runtime.nim` — add `svUninterpRef` arms to every
  ...
  combinator.
- `src/nelli/smt/canonicalize.nim` — add `cacheKeyRaised` proc;
...
- `src/nelli/smt/db.nim` — update callers of renamed suffix constants.
...
- [ ] `InlinePolicy` exported from `types.nim`; `import nelli/smt/types`
  resolves `ipHybrid` without further qualification
- [ ] `svUninterpRef` arm present in every `case sv.kind` dispatch (grep
  confirms: `grep -c 'svUninterpRef' src/nelli/smt/runtime.nim` >= 7)
...
- `src/nelli/smt/runtime.nim` — change `WalkCtx.found` field type
...
macros before the typed AST reaches the nelli symex pipeline.
...
- `docs/symex/templates-macros.md` — new document recording the
  ...
  test harness; (d) the confirmed non-scope: nelli cannot symex
  a SUT whose *name* is computed dynamically by a macro at a call
  site nelli's own macro never sees (the `fn` parameter must
...
nelli's IR/walker/extractor pipeline.
...
- `src/nelli/smt/types.nim` — add `itFloat32`, `itFloat64` to
...
- `src/nelli/smt/dsl_typebridge.nim` — extend `classifyType`'s
...
- `src/nelli/smt/dsl_parser.nim` — extend `emitIRType` with
...
- `src/nelli/smt/runtime.nim` — add `svFloat32` and `svFloat64`
...
- `src/nelli/symex.nim` — extend `primTyAndReader` with
...
- `src/nelli/smt/canonicalize.nim` — extend the type-encoding
...
- `src/nelli/smt/types.nim` — add `iekFloatLit` to `IRExprKind`;
...
- `src/nelli/smt/dsl_parser.nim` — in `parseExpr`, recognize:
...
- `src/nelli/smt/runtime.nim` — add `walk` case for
...
nelli SUT (no `rmRTZ(...)` syntax in Nim property bodies);
...
- `src/nelli/smt/types.nim` — No new `IRBinop` or `IRUnop`
...
- `src/nelli/smt/runtime.nim` — In the walker's `iekBinop`
...
- `src/nelli/smt/dsl_parser.nim` — No parser changes needed;
...
- `src/nelli/smt/runtime.nim` — In `walk(iekBinop)`, extend the
...
- `src/nelli/smt/types.nim` — Add `iekConvIntToFloat` and
...
- `src/nelli/smt/dsl_parser.nim` — Recognize `nnkConv` and
...
- `src/nelli/smt/runtime.nim` — Walk cases for the two new
  expression kinds:
...
- `src/nelli/smt/stdlib_models.nim` — Add a `mathFpModels`
...
- `src/nelli/smt/runtime.nim` — In the call-dispatch path for
...
(though nelli models only a single canonical NaN per
ADR-0005).
...
- `src/nelli/smt/runtime.nim` — Extend `RawWitness` with two
  new tables:
  ```nim
...
- `src/nelli/symex.nim` — Remove F1 stubs; implement
...
- `src/nelli/smt/canonicalize.nim` — Extend the rendering of
...
`src/nelli/smt/canonicalize.nim`, `dsl_parser.nim`, or
...
- [ ] **Walker version bumped `"4"→"5"` atomically as the last
      ...
      `walkerVersion` constant in `src/nelli/smt/canonicalize.nim`
...
- `src/nelli/smt/runtime.nim` — Confirm the `iekArrayGet` path's
...
- `src/nelli/smt/canonicalize.nim` — Confirm that the array
...
- `src/nelli/smt/runtime.nim` — Extend `allocateSym` dispatch:
  ```nim
  of itSeq:
    case e.elemKind
...
- `src/nelli/symex.nim` — Extend `emitTyAndReader` with:
  ```nim
  of itSeq:
    case e.elemKind
...
- `src/nelli/smt/runtime.nim` — Extend the variant arm
...
- `src/nelli/smt/dsl_typebridge.nim` — Confirm `classifyType`
...
**GREEN:**
- `src/nelli/smt/types.nim` — new `IEKind` variants:
...
- `src/nelli/smt/stdlib_models.nim` — new `StdlibModelKind` variants
...
- `src/nelli/smt/dsl_parser.nim` — audit: `getStdlibModelFor` called for
...
**DoD:**
- [ ] `tphase15_S1_typebridge.nim` passes in the nelli dev container (`nimlang/nim:2.2.0` + Z3 4.15.0).
...
**GREEN:**
- `src/nelli/smt/dsl_parser.nim` — confirm `nnkStrLit` → `mkStrLit(e.strVal)`.
...
- `src/nelli/smt/runtime.nim` — the existing `of iekStrLit: SymVal(kind: svString, str: mkString(e.sval))` branch is confirmed correct; no change needed.
...
**GREEN:**
- `src/nelli/smt/stdlib_models.nim` — `smkStrLen`, `smkStrAt`,
  `smkStrSubstr` registered in `getStdlibModelFor(itString)`.
- `src/nelli/smt/dsl_parser.nim` — `s.len` call on an `itString` receiver
...
- `src/nelli/smt/runtime.nim` — `of iekStrLen:` branch: `SymVal(kind: svInt, zi: len(sv.str))`.
...
**GREEN:**
- `src/nelli/smt/stdlib_models.nim` — `smkStrContains`, `smkStrStartsWith`,
  `smkStrEndsWith`, `smkStrFind` registered.
- `src/nelli/smt/dsl_parser.nim` — call interception for `contains`,
...
- `src/nelli/smt/runtime.nim` — lowers:
...
**GREEN:**
- `src/nelli/smt/symex_settings.nim` (or equivalent settings record) —
  add `maxSplitParts: int = 8`.
- `src/nelli/smt/stdlib_models.nim` — `smkStrReplace`, `smkStrReplaceAll`,
...
- `src/nelli/smt/runtime.nim` — lowers:
  ...
  elements.
- `src/nelli/smt/dsl_parser.nim` — intercepts `replace`, `replaceAll`,
...
**What it does:** Creates `src/nelli/smt/regex_parser.nim` as a standalone
...
**GREEN:**
- `src/nelli/smt/regex_parser.nim` (new file) — implements
...
**GREEN:**
- `src/nelli/smt/types.nim` — `iekStrMatch` and `iekStrFindRe` carry
  `rePatternStr: string` field (raw pattern from `re"..."` macro). No recursive
  `IRRegex` type.
- `src/nelli/smt/dsl_parser.nim` — intercepts `match(s, re"pattern")` and
...
- `src/nelli/smt/runtime.nim` — `of iekStrMatch:`/`of iekStrFindRe:`:
...
**GREEN:**
- `src/nelli/smt/types.nim` — `iekStrBytes` IR expression kind (stubbed in S1);
  emitted as `iekStrBytes(recv: IRExpr)`.
- `src/nelli/smt/dsl_parser.nim` — intercepts `bytes(s)` call on `itString`
  receiver; emits `iekStrBytes`.
- `src/nelli/smt/symex_settings.nim` — add `maxBytesEncodingLen: int = 32`.
- `src/nelli/smt/runtime.nim` — `of iekStrBytes:` branch: checks whether
...
**GREEN:**
- `src/nelli/smt/types.nim` — `iekStrConcat` IR expression kind (stubbed in S1).
- `src/nelli/smt/dsl_parser.nim` — intercepts `nnkInfix` with `&` operator
  where both operands are `itString`; emits `iekStrConcat(lhs, rhs)`.
- `src/nelli/smt/runtime.nim` — `of iekStrConcat:` branch:
...
**GREEN:**
- `src/nelli/smt/dsl_parser.nim` — intercepts `toLower(s)` and `toUpper(s)`
  (and dot forms `s.toLower`, `s.toUpper`) on `itString` receivers; emits
  `iekStrUnsupported` node carrying the op name.
- `src/nelli/smt/runtime.nim` — `of iekStrUnsupported:` branch: pushes
...
**GREEN:**
- `src/nelli/smt/dsl_parser.nim` — intercepts `$(n)` where `n` is `itInt`;
  intercepts `parseInt(s)` where `s` is `itString`.
- `src/nelli/smt/runtime.nim` — `of iekIntToStr:` →
...
**GREEN:**
- `src/nelli/smt/runtime.nim` — `of iekStrToInt:` extended with raises-path
  ...
  `"parseFloat"`.
- `src/nelli/smt/dsl_parser.nim` — add interception of `$(f)` / `parseFloat(s)`.
...
**GREEN:**
- `src/nelli/smt/dsl_parser.nim` — intercepts `nnkAsgn` where the LHS is
  ...
  `opName = "string add"`.
- `src/nelli/smt/runtime.nim` — existing `of iekStrUnsupported:` branch
  handles both (per S9 infrastructure).
- `src/nelli/smt/canonicalize.nim` — walker version constant updated: `"5"` → `"6"`.
...
- **`src/nelli/smt/runtime.nim`** — extend the `Path` type with three
  ...
  ```
  grep -n "Path(" src/nelli/smt/runtime.nim
  ```
...
- **`docs/symex/ADR-0010-logical-heap.md`** — authored at this path,
  ...
  *Alternatives considered and rejected.*
  ...
  - **Andersen-style points-to analysis:** whole-program flow-insensitive
    ...
    Andersen requires a whole-program view the nelli engine does not
...
- [ ] `tphase15_H1_path_heap_fields.nim` passes in the nelli dev
...
- `src/nelli/smt/types.nim` — extend `IRStmtKind` with `isRaise`
...
- `src/nelli/smt/dsl_parser.nim` — add cases for `nnkRaiseStmt`,
...
- `src/nelli/smt/runtime.nim` — add `HandlerFrame` and `ExnRecord`
...
- `src/nelli/smt/runtime.nim` — add `sxRaised` to `SymexStatusKind`.
...
- `src/nelli/smt/canonicalize.nim` — add:
  ```nim
  proc cacheKeyRaised*(typeId: string): string =
    ":raised:" & typeId
  ```
...
- `src/nelli/engine/types.nim` — add `sfRaised` to
...
- `src/nelli/symex.nim` — add `of sxRaised:` arms to:
...
- `src/nelli/smt/runtime.nim` — introduce `InternalVerdict` private union:
  ```nim
...
- `src/nelli/smt/runtime.nim` — implement `walk(isTry)`:
...
- `src/nelli/smt/dsl_typebridge.nim` (or a new
  `src/nelli/smt/exn_hierarchy.nim`) — define `ExnTypeTable`:
...
- `src/nelli/smt/runtime.nim` — replace the exact `typeId ∈
...
- `src/nelli/smt/dsl_parser.nim` — after collecting `nnkExceptBranch`
...
- `src/nelli/smt/runtime.nim` — extend `isSubtypeOf` to consult
...
- `src/nelli/smt/runtime.nim` — replace the E3 finally stub with
...
- `src/nelli/engine/types.nim` — add `DefectKind` enum:
...
- `src/nelli/smt/types.nim` — `sxRaised` branch of `RawResult`
...
- `src/nelli/smt/runtime.nim` — in `walk(isRaise)`, after
...
- `src/nelli/symex.nim` — `sfRaised` finding rendering: include
...
- `src/nelli/smt/runtime.nim` — `walk(isRaise)` re-raise branch
...
- `src/nelli/smt/canonicalize.nim` — bump `walkerVersion` from
...
- `src/nelli/smt/runtime.nim` — add walker cases for the magic calls
...
call site before nelli's macro sees the typed AST. Per-call-site
...
**GREEN:**
- `src/nelli/smt/types.nim`: add `isGenericCall` to `IRStmtKind`
...
- `src/nelli/smt/canonicalize.nim`: add `of isGenericCall:` encoding
  and round-trip.
- `src/nelli/smt/runtime.nim`: add `of isGenericCall:` stub to every
...
**GREEN:**
- `src/nelli/smt/dsl_parser.nim`: extend `ensureProcRegistered` to
...
- **Instantiation cache** — keyed by
  ...
  `when defined(nelli_testing): cacheHitsFor(ctx, key): int` accessor
...
**GREEN:**
- `src/nelli/smt/dsl_typebridge.nim`: audit `classifyType` for the
...
- `src/nelli/smt/dsl_parser.nim`: no structural changes; audit only.
...
`emitTyAndReader` / `primTyAndReader` dispatch in `src/nelli/symex.nim`
...
**GREEN:**
- `src/nelli/smt/types.nim`: add `itDistinct` to `IRTypeKind` with
...
- `src/nelli/smt/dsl_typebridge.nim`: in `classifyType`'s `nnkSym`
...
- `src/nelli/smt/dsl_parser.nim`: add `of itDistinct:` to
  `emitIRType` emitting `newCall(bindSym"tDistinct", ...)`.
- `src/nelli/smt/runtime.nim`: add `of svDistinct:` to `SVKind`;
...
- `src/nelli/smt/canonicalize.nim`: add `of itDistinct:` encoding
  `"distinct(" & ty.distinctName & ";" & canonicalize(ty.distinctBase)
  & ")"`.
- `src/nelli/symex.nim`: add `itDistinct` case to `emitTyAndReader` /
...
**GREEN:**
- `src/nelli/smt/dsl_parser.nim`: in `ensureProcRegistered`, detect
...
- `src/nelli/smt/runtime.nim`: add `isDistinctBorrowCall` handling
...
- `src/nelli/smt/dsl_typebridge.nim`: no structural changes; the
...
that never reaches nelli's macro. This cycle specifies and tests the
...
- **User-defined concepts**: trust the Nim semchecker. The semchecker
  verified the constraint at the call site before nelli's macro saw
  the typed AST; nelli does not re-validate. `geConceptViolation`
...
**GREEN:**
- `src/nelli/smt/runtime.nim`: in `of isGenericCall:` dispatch, add
...
- `src/nelli/smt/types.nim`: add `conceptConstraints: seq[string]`
  to `ProcSig`; add `geConceptViolation` to `SymexErrorKind`.
- `src/nelli/smt/dsl_parser.nim`: extend `parseCalleeImpl` to parse
...
**GREEN:**
- `src/nelli/smt/dsl_parser.nim`: extend `gatherTypeSubst` to
...
- `src/nelli/smt/types.nim`: no changes needed (the key is a
...
**GREEN:**
- `src/nelli/smt/dsl_parser.nim`: `gatherTypeSubst` already
...
- `src/nelli/smt/canonicalize.nim`: bump walker version constant
...
`map`, and `fold` over `seq[T]` in nelli's own DSL layer, C4 mixes two
...
**GREEN:**
- `src/nelli/smt/types.nim` — add `iekLambda` and `iekClosureCall` to
  ...
  of iekClosureCall:              ## A-normalised like isCall
    ...
    ccArgs*:    seq[IRExpr]
  ```
- `src/nelli/smt/runtime.nim` — add `svClosure` to `SVKind`:
  ```nim
...
- `src/nelli/smt/dsl_parser.nim` — extend `parseExpr` to detect
  ...
  (A-normalised).
- `src/nelli/smt/canonicalize.nim` — add encoding for `iekLambda`
  (`"Lam:site=<siteHash>/<declOrder>;caps=[<names>];params=[<types>]"`) and
  `iekClosureCall` (`"CC:<callee>(<arg_keys>)"`).
- `src/nelli/smt/dsl_typebridge.nim` — add `classifyProcTy` helper that
...
**GREEN:**
- `src/nelli/smt/runtime.nim` — implement `walk(iekLambda)`: iterate
  ...
  rather than silent).
- `src/nelli/smt/types.nim` — `svClosure` fields:
  ```nim
  of svClosure:
  ...
  ```
- `src/nelli/smt/dsl_typebridge.nim` — no changes (IR already carries
  enough type info from C1).
- `src/nelli/symex.nim` — `emitTyAndReader` stub for closure result
...
**GREEN:**
- `src/nelli/smt/runtime.nim` — implement `walk(iekClosureCall)`: env
...
- `src/nelli/smt/types.nim` — add `maxClosureInlineCount*: int = 64`
  to `SymexSettings`.
...
**GREEN:**
- `src/nelli/smt/dsl_parser.nim` — in `parseExpr`, detect when a
  ...
  new IR table needed.
- `src/nelli/smt/runtime.nim` — no new walker code: `walk(iekLambda)`
...
- `src/nelli/smt/canonicalize.nim` — no change: the site key already
...
**What it does:** Adds walker special-cases for nelli's own DSL
...
**GREEN:**
- `src/nelli/smt/runtime.nim` — add `walkHofFilter`, `walkHofMap`,
...
- `src/nelli/smt/types.nim` — add to `SymexSettings`:
  ```nim
...
- `src/nelli/smt/canonicalize.nim` — include `inlinePolicy` and
  `seqInlineThreshold` in the settings digest.
- `src/nelli/symex.nim` — no change (HOF handling is entirely walker-
  side).
...
**GREEN:**
- `src/nelli/smt/runtime.nim` — in `walk(iekBinop)`, add `of bEq, bNe:`
...
- No new source files. The RED tests drive any fixes needed in:
  - `src/nelli/smt/runtime.nim` — if `WalkerStatics.closureSyms` and
    `WalkCtx.instantiationCache` share mutable state across a generic
    instantiation boundary, separate their scopes.
  - `src/nelli/smt/dsl_parser.nim` — if `parseProc` for a generic
...
**Why a logical heap, not static points-to analysis.** The nelli
...
- Version bumps (walker `"9"→"10"`, rendering `"2"→"3"`) land atomically
  in R12's GREEN only. Both bumps edit `src/nelli/smt/canonicalize.nim`.
...
New IR nodes added to `src/nelli/smt/types.nim`:
...
New `SVKind` variants added to `src/nelli/smt/runtime.nim`:
...
`classifyType` in `src/nelli/smt/dsl_typebridge.nim` gains:
...
`src/nelli/smt/dsl_parser.nim` stubs for `nnkDerefExpr` /
...
- `src/nelli/smt/types.nim` — IR additions above.
- `src/nelli/smt/runtime.nim` — `svRef`/`svPtr` SVKind variants +
  exhaustive stubs. Walker stubs for `itRef`/`itPtr`/`isDeref`/`isNew`
  emit `heUnresolvedRef` (`sevError`) classified error.
- `src/nelli/smt/dsl_typebridge.nim` — `classifyType` extensions
  including `owned T`/`WeakRef`/`Atomic` → `heUnsupportedOwnership`.
- `src/nelli/smt/dsl_parser.nim` — deref/new parse stubs.
...
- [ ] `nim c src/nelli/smt/runtime.nim` succeeds with no new warnings
...
- `src/nelli/smt/runtime.nim` — promote `svRef`/`svPtr` stubs from R1a
...
- `src/nelli/smt/dsl_typebridge.nim` — promote `nnkRefTy`/`nnkPtrTy`
  stubs from R1a to full `tRef(pointeeTy)` / `tPtr(pointeeTy)` emission.

- `src/nelli/smt/dsl_parser.nim` — promote `nnkDerefExpr` /
...
- `src/nelli/symex.nim` — `emitTyAndReader` gains a case for `itRef` /
...
- `src/nelli/smt/runtime.nim` — at each `isCall`/`isGenericCall`/
...
- `src/nelli/smt/runtime.nim` — implement `of isNew:` walker branch.
...
- `src/nelli/smt/runtime.nim` — `of isDeref:` already stubs select
...
- `src/nelli/smt/types.nim` — add `isDerefWrite` IR statement kind
...
- `src/nelli/smt/runtime.nim` — implement `of isDerefWrite:`.
...
- `src/nelli/smt/types.nim` — new `SymexTargetKind.stkNilAccess`
...
- `src/nelli/smt/runtime.nim` — `of isDeref:` (and `isDerefWrite:`)
...
- `src/nelli/smt/dsl_parser.nim` — extend `parseExpr` to recognise
...
- `src/nelli/smt/runtime.nim` — implement field-modify helper:
...
- `src/nelli/smt/runtime.nim` — inherited field resolution
...
- `src/nelli/smt/runtime.nim` — `of isLet:` for `svRef`/`svPtr`
...
- `src/nelli/smt/runtime.nim` — `svPtr` SymVal variant already
...
- `src/nelli/smt/dsl_parser.nim` — in `parseStmt`, recognise `nnkCall`
...
- `src/nelli/smt/runtime.nim` — at `isCall`/`isGenericCall` walker
...
- `src/nelli/smt/runtime.nim` — `of isDeref:` and `of isDerefWrite:`
...
- `src/nelli/smt/canonicalize.nim` — add `maxHeapDepth` to the
...
- `src/nelli/smt/types.nim` — new `IRStmtKind.isUnsafeCast` with
...
- `src/nelli/smt/dsl_parser.nim` — in `parseExpr` / `parseStmt`,
...
- `src/nelli/smt/runtime.nim` — `of isUnsafeCast:` sets
...
1. **Walker version bump `"9"→"10"`.** Per v2 invariant 1, Cluster R's
   ...
   `src/nelli/smt/canonicalize.nim` (single source of truth per M12).
...
2. **Rendering version bump `"2"→"3"`.** The heap-snapshot section is new
   ...
   backward compatibility. This bump also edits `src/nelli/smt/canonicalize.nim`
...
- `src/nelli/smt/canonicalize.nim` — bump both version constants:
  `walkerVersion = "10"` and `renderingVersion = "3"`. These are the
  single source-of-truth constants (per M12; not in `runtime.nim`).

- `src/nelli/symex.nim` — extend the on-disk witness serialiser to
...
- `src/nelli/smt/runtime.nim` — `itLambda` arm: accept `svRef`/`svPtr`
...
- `src/nelli/smt/runtime.nim` — `extractFromSymVal` for `svClosure`:
