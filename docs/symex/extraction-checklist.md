...
- A non-PBT consumer adopts symex (e.g. a static analyser, a
  ...
  proptest's engine).
- The proptest maintainers want to ship symex on its own release
  cadence.
- The substrate (`nim-z3`) version proptest needs lags or leads
  what symex needs.

Single-criterion extraction is premature. The `symex/` package is
small enough that carrying it inside proptest costs nothing today.
...
| proptest-shared (extraction targets) | 3 |
| substrate (`z3`) | 2 |
| nim-stdlib | 19 |

The three proptest-shared imports are the entire surface to be
re-routed for extraction.

## The three extraction targets

### 1. `choice` (`proptest/choice`)
...
"recorded primitive draw". It belongs to proptest's PBT engine
...
shared package — e.g. `nim-choice-ir` — that both `proptest` and
`nim-symex` depend on. Effort: half a day; the file is ~400 lines
and has no transitive proptest deps.

**Alternative**: define a local `SymexChoice` type in `nim-symex`,
with an adapter on proptest's side that converts. Decoupled but
...
Adapter on proptest's side wraps `ExampleDatabase` to satisfy
this protocol. Effort: a few hours.

### 3. `engine/types` (`proptest/engine/types`)
...
imports `symex` thematically — proptest's engine integrates with
symex, not the other way around.

**Recommended path**: move `SymexFinding` to `nim-symex/types`.
Proptest's `engine/types` imports `nim-symex/types` to embed the
...
1. **Extract `nim-choice-ir`** (or commit to the `SymexChoice`
   alternative). Verify proptest tests pass against the extracted
   `choice` import. Tag `nim-choice-ir` v0.1.
2. **Create the `nim-symex` repo** with:
   - `src/symex/` containing the current contents of
     `src/proptest/smt/**` and `src/proptest/symex.nim` (renaming
     `proptest/symex.nim` → `symex.nim`).
...
3. **Reverse the `engine/types` ↔ `SymexFinding` dependency**:
   - Delete `SymexFinding` from `proptest/engine/types.nim`.
   - Import `nim-symex/types` into `proptest/engine/types.nim`.
   - Re-export so existing callers still see `SymexFinding`
     through `proptest/engine`.
4. **Replace `db` import** with the `SymexWitnessStore` protocol +
   proptest-side adapter.
5. **Rename test files** in proptest:
   - `tests/tsymex_*.nim` → these stay in proptest (they test the
     proptest-side integration).
   - Mirror the core fragment tests inside `nim-symex/tests/`.
6. **Update `docs/symex/`** in proptest to reference
   `nim-symex` as the upstream; keep the integration story local.
7. **Tag `nim-symex` v0.1**. Pin to it in proptest's
   `proptest.nimble`.
...
- The Z3 substrate — `nim-z3` is already a separate library,
  pinned by both proptest and the (future) `nim-symex`.
...
target should fail loud if `proptest-shared` grows beyond the
