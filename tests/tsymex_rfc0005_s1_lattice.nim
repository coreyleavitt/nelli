...
suite "RFC-0005 S1 (c) -- RawResult.pathTaint is produced at the target-hit site":
  ...
  test "no verdict change: an isUnsupported (kindless, transitional) path still blocks SAT and degrades the run to sxUnknown":
    let prog = SymexProgram(params: @[],
      body: mkBlock(@[mkUnsupported("rfc0005 S1 probe"), mkTargetLabel("hit")]))
    let raw = runSymex(prog, SymexTarget(kind: stkLabel, label: "hit"))
