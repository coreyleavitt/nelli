...
## C1's slice.
##
## Round A deleted this at A3, once all 115 `Settings` pins were gone. It is
## restored for round B, which needs exactly the same staging for the ten
## `SymexSettings`/`ResourceBudget` const literals, and is deleted again at
## B1b once those pins are resolved. The alternative was hand-writing thirteen
## budget fields plus four top-level ones at each of ten sites, which is the
## same unreadable, judgement-laden edit the macro exists to avoid.
##
## Round B needs it at TWO levels: every one of the ten literals nests a
## partial `ResourceBudget` inside a partial `SymexSettings`, and this macro is
## deliberately not recursive, so both get wrapped explicitly.
